...
description: Pre-approval review loop for PRD/spec markdown files: parallel /codex-review + /opencode-review until clean. Use before presenting a PRD/spec for approval. Trigger: "prd-review-loop", "/prd-review-loop".
...
1. Invoke `/codex-review` and `/opencode-review` on the PRD/spec file **in parallel** (single message, two Skill calls).
...
- **Parallel reviews.** `/codex-review` and `/opencode-review` MUST be dispatched in a single message containing two Skill tool calls.
