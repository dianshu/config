...
Use the Grep and Glob tools to search existing skills:

- `Glob` with pattern `~/.claude/skills/*/SKILL.md` to list all skills
- `Grep` with keywords to search skill content across `~/.claude/skills/` and `.claude/skills/`
- `Grep` with exact error messages (`-F` flag) for precise matching
...
For the standard skill format, refer to the template at `references/skill-template.md` in this skill's directory.
...
**Mirroring**: After saving to `~/.claude/skills/`, copy to `~/repos/config/claude/skills/` to keep the config repo in sync. `~/.claude/` is the single source of truth. Then use /push to commit and push only the mirrored changes in the config repo.
