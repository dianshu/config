...
update_zshrc() {
    ...
    if command -v dl_with_backup >/dev/null 2>&1; then
        ...
        _dl() {
            mkdir -p "$(dirname "$2")"
            curl -fsSL -H 'Cache-Control: no-cache' -o "$2" "$1" && echo "  OK ${2/$HOME/~}" || { echo "  FAILED ${2/$HOME/~}"; return 1 }
        }
    fi
